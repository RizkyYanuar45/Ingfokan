const handleDeleteAuthor = async (bannerId) => {
  const api = import.meta.env.VITE_API_URL;
  // Request hapus tanpa konfirmasi
  let response = await fetch(`${api}/author/delete/${bannerId}`, {
    method: "DELETE",
    headers: {
  ...
  if (result.message.includes("Peringatan")) {
    ...
    if (confirmDelete) {
      ...
      response = await fetch(`${api}/author/delete/${bannerId}?confirm=true`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
